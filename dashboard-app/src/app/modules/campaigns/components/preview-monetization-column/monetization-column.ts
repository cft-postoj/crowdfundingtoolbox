...
export function setActiveButtonMonthlyColumn(chosenButton, focusInput: false) {
    var target;
    ...
    var columns = header.getElementsByClassName('cft--monatization--column-wrapper cft--monatization--only-monthly');
    for (var i = 0; i < columns.length; i++) {
        if (columns[i].classList.contains('active')) {
            columns[i].className = columns[i].className.replace(' active', '');
        }
    }

    var activeColumn = chosenButton.closest('.cft--monatization--column-wrapper');
    activeColumn.className += ' active';
...
export function changePaymentOptionsColumn(element) {
...
export function createBankButtonsColumn(monetizationEl, bankButtonsData) {
        ...
        if (bankButtonsData[i].image == null) {
            bankButtonsWrapper.insertAdjacentHTML('beforeEnd',
                `<div class="bank-button__container"> 
                        <div class="bank-button" data-bank-link="${bankButtonsData[i].redirect_link}" onclick="parent.setBankButtonColumn(this)">
    ...
    if (bankButtonsData.length > 5) {
        var options =
            `<div class="bank-button__container">
                <div class="bank-button bank-button__select" onclick="parent.setBankButtonColumn(this)">
                    <select name="bank">
...
export function handleSubmitColumn(el, event) {
    ...
    let title = monetizationEl.querySelector('.cft--monetization--container-step-1 .cft--monatization--column-wrapper.active .cft--monatization--column-container__title')
    ...
    if (validateFormColumn(el)) {
        ...
        let data = JSON.stringify(
            {
                ...
                'notes': 'category=' + title.innerText + ';'
            }
        );
        ...
        xhr.onload = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                showSecondStepColumn(monetizationEl, xhr.response.variable_symbol, xhr.response.bank_account, xhr.response.bankButtons,
...
export function showSecondStepColumn(monetizationEl, variableSymbol, bankAccount, bankButtons, value, frequency,
    ...
    createBankButtonsColumn(monetizationEl, bankButtons);
    stepColumn(monetizationEl, true);
...
export function stepColumn(element, increase) {
    var monetizationCont = element.closest('.cft--monetization--container');
...
export function validateFormColumn(el) {
...
export function donationInProgressColumn(element) {
    const apiPublicUrl = getEnvs().apiPublicUrl + '/portal/';
    let xhr = new XMLHttpRequest();
    var monetizationCont = element.closest('.cft--monetization--container');
    const data = JSON.stringify(
...
export function setBankButtonColumn(element) {
...
export function trackEmailOnChangeColumn(el) {
...
export function trackInsertValueColumn(chosenButton, frequency, apiUrl: string) {
    let xhttp = new XMLHttpRequest();
    const data = JSON.stringify(
        {
...
export function paymentCountryTypeColumn(element, type) {
    ...
    if (type === 'foreign' && choosedPaymentType.value === 'monthly') {
        ...
        for (let i = 0; i < cardPayOptions.length; i++) {
            ...
            changePaymentOptionsColumn(cardPayOptions[i]);
        }
        ...
        if (monetizationCont.querySelector('.payment-options__button input').value === 2) {
            monetizationCont.querySelector('.payment-option[data-id="1"]').classList.add('cft--monatization--hidden');
            monetizationCont.querySelector('.payment-option[data-id="2"]').classList.remove('cft--monatization--hidden');
            monetizationCont.querySelector('.cft--button-container[data-id="1"]').classList.add('cft--monatization--hidden');
            monetizationCont.querySelector('.cft--button-container[data-id="2"]').classList.remove('cft--monatization--hidden');
